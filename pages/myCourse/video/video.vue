	...
	<view v-if="url">
		...
		<view class="video-box" v-else="">
			<view class="process-box">
				...
				<slider @change="sliderChange" min="0" :max="maxValue" block-size="20" :show-value="false" :value="offerSet" />
				<!-- <progress :percent="offerSet" stroke-width="6" /> -->
				<text>{{ duration }}</text>
			</view>
		...
		methods: {
			init() {
				...
				myaudio.onCanplay(e => {
					...
					console.log('可以播放');
					setTimeout(() => {
						this.maxValue = myaudio.duration;
				...
				myaudio.title = Title;
				myaudio.epname = '幸福谋方课程';
				myaudio.singer = '幸福谋方';
				myaudio.coverImgUrl = BannerPictureUrl;
				this.play()
			},
			//开始播放
			play() {
