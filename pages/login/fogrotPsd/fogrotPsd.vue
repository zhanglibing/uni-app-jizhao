<template>
	<view class='login-box'>
		<view class="row">
			...
			<input @blur="blur" v-model="option.Username" :maxlength="11" type='number' placeholder='请输入账号'></input>
		</view>
		<view class="row sms">
		...
		<view @tap="submitPsdChange" :class="{disabled:!canSubmit}" class="block-btn">确认绑定</view>
	</view>
</template>

<script>
	import {sendSms,changePsd,phoneIsReg} from '../../../api/login.js'
	...
	export default {
		...
		methods:{
			...
			submitPsdChange(){
				...
				changePsd(this.option).then(res => {
					uni.showToast({
						icon:"success",
						title:'密码找回成功'
					})
