<template>
	<view v-if="info">
		<view class="info-box">
			<view class="avatar"> <image :src="info.AvatarUrl" mode=""></image> </view>
			<view class="desc">
				<view class="name-box">
		...
		<view class="server-box">
			...
			<view class="bank">
				<view
					><image src="../../../static/images/bank.png" mode="widthFix"></image
					>银行卡转账</view
				>
				<view
		...
		<view class="tab-box">
			...
			<view class="desc-box">
				<view
					class="intro"
					v-if="active == 0"
					v-html="info.ConsultantInfo.PersonalIntroduction"
				></view>
				<view
					class="background"
					v-if="active == 1"
					v-html="info.ConsultantInfo.CareerBackground"
				></view>
				<!-- 文章模块 -->
				<view v-if="active == 2">
	...
	methods: {
		init() {
			gtConsultantInfo(this.id).then(res => {
				...
				this.info = res;
				// 获取用户评价
				gtListComment({ ConsultantId: this.id }).then(res => {
					let { Total, Data } = res;
					this.comment = Data;
				});
				article.getMyArticle({ customerId: this.id }).then(res => {
